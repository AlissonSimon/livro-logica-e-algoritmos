...
const dvMoedas = document.getElementById('divMoedas');
...
formulario.addEventListener('submit', (event) => {
    ...
    const soma = Number(formulario.soma.value);
    const moedas = dvMoedas.querySelectorAll('img');
    let totalMoedas = 0;

    for (const moeda of moedas) {
        if (moeda.classList.contains('moeda1-00')) {
            totalMoedas += 1;
        } else if (moeda.classList.contains('moeda0-50')) {
            totalMoedas += 0.5;
        } else if (moeda.classList.contains('moeda0-25')) {
            totalMoedas += 0.25;
        } else {
            totalMoedas += 0.1;
        }
    ...
    let mensagem;
    if (soma.toFixed(2) === totalMoedas.toFixed(2)) {
